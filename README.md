...
- Node.js **≥ 22.16** (розроблялось на v24.14.1) — планку задає `import.meta.main`,
  який зʼявився саме в 22.16; ESM вмикається через `"type": "module"` у `package.json`
...
Те саме через npm: `npm start` і `npm run start:https`.

## Обмеження

Блок заголовків обмежений **8 КБ**. Якщо клієнт перевищив ліміт і так і не надіслав
порожній рядок, сервер відповідає `431 Request Header Fields Too Large` і рве зʼєднання —
інакше одне зависле зʼєднання нарощувало б буфер до вичерпання памʼяті процесу.
